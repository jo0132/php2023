...
<body class="gray">
                ...
                <form action="joinResult.php" name="joinResult" method="post" onsubmit="return joinChecks()">
                    <fieldset>
                        ...
                        <div>
                            ...
                            <input type="password" id="youPass" name="youPass" class="inputStyle" placeholder="비밀번호을 적어주세요!" require>
                            <p class="msg" id="youPassComment"></p>
                            <!-- 비밀번호, 특수기호, 숫자가 들어가야합니다. -->
                        </div>
                        <div>
                            <label for="youPassC" class="required">비밀번호확인</label>
                            <input type="password" id="youPassC" name="youPassC" class="inputStyle" placeholder="다시 한번 비밀번호을 적어주세요!" require>
    ...
    <script>
        ...
        function joinChecks(){
            ...
            //닉네임 유효성 검사
            if($("#youNick").val() == ''){
                $("#youNickComment").text("* 낵네임을 입력해주세요!");
                $("#youNick").focus();
                return false;
            }
            ...
            if($("#youPass").val() !== $("#youPassC").val()){
                $("#youPassCComment").text("* 비밀번호가 일치 하지 않습니다.");
                $("#youPassC").focus();
                return false;
            }
            // 생년월일 입력
            ...
            if(!getYouPhone.test($("#youPhone").val())){
                $("#youPhoneComment").text("* 연락처가 정확하지 않습니다.(000-0000-0000)");
                $("#youPhone").val('');
                $("#youPhone").focus();
                return false;
            }
